...
struct EDTDATA//EDT创建数据
{
	...
	int Typename;//标识符WEDT
	int delimiter1;//分隔符0010 65536 也可以两个short，看你怎么写了;
	double  check;//COleDateTime::GetTickCount()，每次打开会刷新，兼容原版EDB;;
	int delimiter2;//分隔符0000 0;//分隔符1000 1;
	BYTE data[492];
};
struct EDBDATA//EDB创建数据
{
	EDBDATA() {
		Typename = 'BDEW'; //标识符WEDB，
		delimiter1 = 0x00010000;//分隔符0010 65536 也可以两个short，看你怎么写了;
		check = 0;//COleDateTime::GetTickCount()，每次打开会刷新，兼容原版EDB;;
		delimiter2[0] = 0;//分隔符0000 0;
		delimiter2[1] = 1;//分隔符1000 1;
		TotalLength = 0;//有效字段StorageLength4+各类型长度*数组数量 长度4;;
		memset(blankBYTE, 0, 84);
		ValidColumnNum = 0;//第112位后储存的int是有效字段数量;
	}
	int Typename;//标识符WEDB
	int delimiter1;//分隔符0010 65536 也可以两个short，看你怎么写了;
	double  check;//COleDateTime::GetTickCount()，每次打开会刷新，兼容原版EDB;;
	int delimiter2[2];//分隔符0000 0;//分隔符1000 1;
